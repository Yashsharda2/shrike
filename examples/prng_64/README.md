...
A hardware random number generator built on the Shrike Lite. A small mixing function runs entirely in FPGA fabric, spinning a 64-bit state on every clock cycle. The RP2040 sends in whatever you type, and the timing of that gets mixed into the state, so the FPGA turns it into a unguessable number which can be used as passwords.
